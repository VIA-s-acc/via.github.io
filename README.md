...
<лимит> - Целое число от 0 до 25. Если вы бог, верхний предел поднимается.

Отображает топ-последователей вашего бога (или вас, если вы бог), до <лимит>.

Формат для этого следующий:
- Место
- Идентификатор пользователя
- Количество благосклонности
- Текущая удача

Результат прикреплен в виде текстового файла.

### Перестать Следовать

`$unfollow`

**Проверки:** has_god, has_character

Отпустите вашего текущего бога и станьте безбожником. Это **навсегда!**

Хотите изменить своего бога? Просто используйте `$follow` снова.

Как только вы станете безбожником, все ваши очки сброса и ваш бог будут удалены.
Становясь безбожником, ваша удача не возвращается к 1,00 немедленно, она изменяется вместе с удачей всех остальных в понедельник.
...
Молитесь своему богу, чтобы получить случайное количество очков благосклонности, варьирующееся от 0 до 1000.

Существует 33% шанс получить 0 благосклонности, 33% шанс получить от 0 до 500 благосклонности и 33% шанс получить от 500 до 1000 благосклонности.
...
**Проверка:** cooldown, has_character

Выберите или измените свою расу. Вы можете выбрать это, пока у вас есть оставшиеся очки сброса.

Каждая раса имеет различное распределение урона/защиты:
- Орк: +4 защиты, +0 урона
- Дварф: +3 защиты, +1 урона
- Человек: +2 защиты, +2 урона
- Эльф: +1 защиты, +3 урона
- Джиккл: +0 защиты, +4 урона

По умолчанию вы человек.

После выбора расы вам будет задан личный вопрос, ответ на который может повлиять на что-то.
...
Запускает голодные игры

Игроки смогут присоединиться с помощью эмодзи 🍽️.
Игра управляется как случайными действиями, так и, возможно, выбранными действиями.
Игроки могут выбирать действие, если получат прямое сообщение от бота. Если игрок не выберет действие, бот выберет его за него.

Не каждому игроку будет дана возможность выбирать действие. Иногда никто не сможет выбрать, так что не отчаивайтесь.

## Maths

### Калькулятор

`$[calculate|math|maths|calc] <выражение>`

`<выражение>` - математическое выражение для вычисления

Вычисляет что-то. pi - это pi, tau - это tau, а e - это math.e
Поддерживает round(), sin(), cos(), sqrt(), tan() и бесконечность (inf) и NaN (nan).
Работает с присваиванием переменных и многострочными выражениями.

## Альянсы

### Приглашение В Альянс

`$alliance invite <новый_лидер>`

**Проверки:** has_character, is_alliance_leader, cooldown

`<новый_лидер>` - Пользователь Discord (Глава какой-либо гильдии) 

Пригласите гильдию в свой альянс. Все союзные гильдии будут получать выгоду от зданий вашего города. После того как вы объединитесь с другой гильдией, это будет показано в `$alliance`.
Другая гильдия не может быть союзницей другого альянса или владеть городом, чтобы быть приглашенной.

*Эту команду может использовать только лидер альянса.*

*(Эта команда имеет перезарядку в 5 минут.)*

### Создание Альянса

`$alliance build`

Постройте здания `$alliance build building` или защиту `$alliance build defense`.

### Постройка Зданий

`$alliance build building <название>`

**Проверки:** has_character, is_alliance_leader, owns_city, cooldown

`<название>` - Название здания для улучшения.

Улучшите одно из зданий вашего города, предоставляющее лучшие пассивные бонусы. Максимальный уровень любого здания - 10.
В зависимости от доступных зданий города, `<название>` может быть Thief, Raid, Trade или Adventure. Используйте `$alliance buildings`, чтобы увидеть, какие доступны.

Стоимость улучшения зависит от следующего уровня здания и рассчитывается как следующий_уровень * $100,000.
Стоимость улучшения будет списана с банка гильдии лидера альянса.

*Для использования этой команды ваш альянс должен владеть городом.*
*Эту команду может использовать только лидер альянса.*
...
`$alliance attack <город>`

**Проверки:**  is_guild_leader, cooldown

`<город>` - Название города. Вы можете проверить названия городов с помощью `$cities`

Атакуйте город, снижая его защиту, чтобы, возможно, захватить его.
Атака на город активирует период милости в 12 часов, в течение которого его нельзя атаковать снова.
Инициирование атаки будет стоить деньги гильдии лидера альянса, в зависимости от зданий в защищающемся городе.

При использовании этой команды бот отправит сообщение с кнопкой, используемой для присоединения к атаке. Каждый член альянса может присоединиться.
Через десять минут после отправки сообщения присоединившиеся пользователи будут собраны, их атака и защита будут зависеть от их экипировки, класса и бонусов за рейды, а также их статистики рейдов, и начнется атака.

Во время атаки сначала будут атакованы защиты с наибольшим количеством HP. Весь урон атакующих будет суммироваться.
Сумма урона защиты и наносится либо на атакующего с наименьшим HP, либо на атакующего с наибольшим уроном.

Если защита достигает нулевого HP, она будет удалена из города, и ее HP не восстановится после окончания атаки.
Атакующие, достигшие нулевого HP, также будут удалены из атаки.

Если защита города была разрушена, ваш альянс может немедленно занять город (`$alliance occupy`)

*Эту команду может использовать только лидер альянса.*

*(Эта команда имеет перезарядку в 2 часа.)*
...
Перечисляет все здания в вашем городе, а также их уровень. Эти здания приносят пассивные награды всем членам альянса:

Здания Thief увеличивают ваш шанс украсть деньги вором, за каждый уровень ваш шанс увеличивается на 5%
Здания Raid увеличивают raidstats пользователя на 0.1 за уровень
Здания Trade удаляют необходимость платить налог в 5% при покупке или продаже предметов, когда он достигает хотя бы уровня 1. Он также увеличивает количество денег, которое вы получаете от $merch и $merchall, увеличивая награду на 50% за каждый уровень
Здания Adventure уменьшают время приключения на 1% за уровень и увеличивают ваши шансы на успех на 1% за уровень.

Ваш альянс должен владеть городом, чтобы использовать эту команду.

### Кик Из Альянса

`$alliance kick <гильдия_для_исключения>`

**Проверки:** has_character, is_alliance_leader

`<гильдия_для_исключения>` - Название гильдии или ее ID

Исключите гильдию из вашего альянса. После исключения гильдии она больше не будет получать пассивные бонусы от зданий владеющего города.
...
**Проверки:** has_character, is_alliance_leader, owns_city

Отказ от вашего города немедленно заставит всех членов альянса потерять все пассивные бонусы, предлагаемые зданиями города, и владение городом будет передано обществу Гильдийных Альянсов.
...
`$alliance occupy <город>`

**Проверки:** has_character, is_alliance_leader, owns_no_city

`<город>` - Название города. Вы можете проверить названия городов с помощью `$cities`

Занять город. После этого ваш альянс будет владеть этим городом и сможет строить защиту и улучшать здания.
Вы можете занять город только в том случае, если у него не осталось защиты.

Занятие города устанавливает перезарядку на 10 минут, в течение которой его нельзя захватить другим альянсом.
Занятие города также сбрасывает все его здания на уровень 0.

Вы не можете занять город, если ваш альянс уже владеет им.
*Эту команду может использовать только лидер альянса.*
...
Перечисляет оборонительные сооружения вашего города и показывает оставшийся HP для каждой из них.
...
Перечисляет специфические для альянса перезарядки, что означает, что все члены альянса имеют эти перезарядки и не могут использовать команды.

### Постройка защиты

`$alliance build defense <название>`

*Проверки: has_character, is_alliance_leader, owns_city, cooldown

Постройте защитные сооружения или разместите войска в ваших городах. Доступны следующие варианты:

Cannons (пушки): 500HP, 60 защиты за $200,000
Archers (лучники): 1,000HP, 50 защиты за $100,000
Outer Wall (наружная стена): 40,000HP, 0 защиты за $500,000
Inner Wall (внутренная стена): 20,000HP, 0 защиты за $200,000
Moat (ров): 10,000HP, 25 защиты за $150,000
Tower (башня): 2,500HP, 50 защиты за $200,000
Ballista (баллиста): 500HP, 30 защиты за $100,000

Любой город может иметь максимум 10 защит. При атаке сначала будут атакованы здания с наибольшим количеством HP.
Вы не можете строить защиту, пока ваш город находится под атакой. Стоимость защиты будет вычтена из банка гильдии-лидера.

*Для использования этой команды ваш альянс должен владеть городом.*
*Эту команду может использовать только лидер альянса.*

*(У этой команды перезарядка в 1 минуту)*

### Города

`$cities`

Показывает все города, их уровни, владельцев, доступные здания и текущую защиту.
...
Покиньте свой альянс. После вашего отъезда из альянса вы больше не будете получать пассивные бонусы от владения городом.

Если вы являетесь лидером альянса, вы не можете покинуть его (рассмотрите` $alliance kick`).

Только лидеры гильдий могут использовать эту команду.

### Альянс

`$alliance`

**Проверки:**  has_guild, has_character

Альянсы - это группы гильдий. Как и для гильдии, требуется хотя бы один член, для альянса требуется хотя бы одна гильдия и считается одногильдийным альянсом.
Альянсы могут занимать города для получения пассивных бонусов от зданий.

Если эта команда использована без подкоманды, она показывает ваши союзные гильдии.
См. `$help` alliance для списка команд для взаимодействия с вашим альянсом!
...
Показывает все кластеры и их текущий статус.

### Таймеры

`$[cooldowns|t|cds|timers] `

Показывает все ваши перезарядки, включая ваш таймер приключения.

## Викторина

### trivia

`$[tr|trivia] [сложность=легкая]`

`[сложность]` - Сложность вопроса, может быть легкая, средняя или сложная

Ответьте на вопрос из OpenTDB. Вы выбираете свой ответ с помощью реакций эмодзи.
...
Используется для просмотра списка доступных скоростей игры "Оборотни".

### Роли

`$werewolf roles [роль]`

Просмотр описаний ролей в игре "Оборотни".
`$roles` для просмотра всех ролей.
`$roles <название роли здесь>` для просмотра информации о роли.

### Посмотреть Роль

`$werewolf myrole `

Проверьте свою роль в игре "Оборотни" и позвольте боту отправить ее вам в личном сообщении.

Для получения своей роли вы должны быть частью текущей игры.

### Режимы

`$werewolf modes `

Используется для просмотра списка доступных режимов игры "Оборотни".

### Играть

`$[ww|werewolf] [режим=Классический] [скорость=Нормальная] [мин_игроков]`

`[режим]` - Режим игры, см. ниже доступные варианты. (необязательно и по умолчанию Классический)
`[скорость]` - Скорость игры, см. ниже доступные варианты. (необязательно и по умолчанию Нормальная)
`[мин_игроков]` - Минимальное количество игроков для игры. (необязательно и по умолчанию в зависимости от режима игры: Классический: 5, Несбалансированный: 5, Охотник: 8, Житель: 5, Валентин: 8, IdleRPG: 5)

Начинает игру в "Оборотни". Найдите оборотней, прежде чем они найдут вас!
Ваша цель для победы указана на вашей роли.

Режимы игры: Классический (по умолчанию), Несбалансированный, Охотник, Житель, Валентин, IdleRPG. Используйте `$ww modes` для получения подробной информации.
Скорости игры (в секундах): Нормальная: 60 (по умолчанию), Расширенная: 90, Быстрая: 45, Блиц: 30. Используйте `$ww speeds` для получения подробной информации.
...
`$ww Blitz` для режима Классика на скорости Блиц
`$ww Imbalanced`  Несбалансированный для режима Несбалансированный на скорости Нормальная
`$$ww Valentines Extended` для режима "Valentines" на увеличенной скорости
`$ww Huntergame Fast` Быстрая для режима Охотник на скорости Быстрая
...
Показывает ваш ELO и рейтинг лучших шахматистов, ограниченный шахматами IdleRPG.

### Ходы

`$chess match moves `

Показывает ходы текущего матча в канале, которые сделали вы и ваш оппонент.

### Матч

`$chess match [сложность=3] [противник]`

`[сложность]` - Целое число от 1 до 10; по умолчанию 3
`[противник]` - Пользователь; по умолчанию никто

Начинает партию в шахматы.

Если указана `сложность`, вы будете играть против искусственного интеллекта Stockfish с указанной сложностью.
Если указан `противник`, вы будете играть против этого противника, независимо от того, указали ли вы сложность или нет.

Вы можете выбрать сторону, за которую хотите играть, используя `эмодзи` в начале игры.
Если вы играете против `противника`, и вы оба зарегистрированы в ELO, вам будет предложено выбрать рейтинговую или нет.

В одном канале может быть только одна игра в шахматы одновременно.

Ходы в шахматах можно отправлять в нескольких форматах:
-g1f3
-Nf3 ⚠
-0-0
-xe3

Имейте в виду, что они чувствительны к регистру:
Фигуры - заглавные буквы:
-король = K
-Ферзь = Q
-слон = B
-конь = N
-Ладья = R
-пешка = без обозначения
Поля - строчные буквы.
...
Регистрирует учетную запись, подходящую для ELO-рейтинга, для шахмат Idle's.
Рейтинг определяет ваш относительный уровень мастерства и может увеличиваться/уменьшаться при выигрыше/проигрыше.

### Шахматы IdleRpg

`$chess `

Система шахмат IdleRPG. Вы можете играть против ИИ или других пользователей и повышать свой ELO.

## Изображения

### edgy

`$edgy [пользователь=Автор]`

`[пользователь]` - Пользователь Discord, чей аватар хотите изменить; по умолчанию сам автор

Находит и усиливает края на аватаре пользователя, создавая крутой эффект изображения.

### pixelfy

`$pixelfy [пользователь=Автор] [размер=2]`

`[пользователь]` - Пользователь Discord, чей аватар нужно пикселизировать; по умолчанию сам автор
`[размер]` - Степень пикселизации, может быть числом от 1 до 5; по умолчанию 2

Пикселизирует аватар пользователя. Если у пользователя анимированная иконка, используется первый кадр.

### invert

`$invert [участник=Автор]`

`[участник]` - Пользователь Discord, чей аватар нужно инвертировать; по умолчанию сам автор

Инвертирует цвета на аватаре пользователя.

(Эта команда имеет кулдаун канала 15 секунд.)

### oil

`$oil [участник=Автор]`

`[участник]` - Пользователь Discord, чей аватар нужно нарисовать маслом; по умолчанию сам автор

Создает эффект масляной живописи на аватаре пользователя.
...
`$snowballfight <противник> [игроки=10]`

Устроить снежки против другой гильдии.
...
Переключает автоматические напоминания, когда вы заканчиваете приключение.

### Отменить Напоминание

`$remind [remove|rm|delete|del|cancel] <идентификатор>`

`[идентификатор]` - Идентификатор напоминания

Отменяет запущенное напоминание по его идентификатору.

Чтобы найти идентификатор напоминания, используйте `$reminder list`.

### Список Напоминалок

`$reminders `

Показывает список ваших текущих запущенных напоминаний

Напоминания можно отменить с помощью `$reminder cancel <идентификатор>.`

(служит псевдонимом для `$reminder list`)

### Напоминалка

`$[r|reminder|remindme|remind] <когда_и_что>`

`<когда_и_что>` - Тема и время напоминания, см. ниже для получения дополнительной информации.

Напомните себе о чем-то, что вам нужно сделать в будущем.

`<когда_и_что>` может быть вашим напоминанием и временем, принимаются несколько форматов:

`- $remind 12h` голосовать на top.gg
`- $remind 12am` использовать $daily
`- $remind next monday` посмотреть нового Бога

Пожалуйста, держите его в порядке `$remind time subject`, чтобы это работало правильно

### Список Напоминалок V2 
...
**Проверка:** has_no_transaction

Открывает сессию торговли для вас и другого игрока.
Используя `$trade <пользователь>`, затем пользователь, принимающий флажок, начнет сессию торговли.

Пока сессия торговли открыта, вы и другой игрок можете добавлять или удалять предметы, деньги и кейсы по своему выбору.

Вот несколько примеров, чтобы познакомить вас с концепцией:
`- $trade add crates 10 common`
`- $trade set money 1000`
`- $trade remove item 13377331` (это работает только если вы добавили этот предмет ранее)
`- $trade add items 1234 2345 3456`

Чтобы принять сделку, оба игрока должны реагировать с эмодзи ✅.
Принятие сделки переведет все предметы в сессии торговли другому игроку.

Вы не можете торговать сами с собой или иметь более одной открытой сессии торговли одновременно.
Передача любых предметов, кейсов или денег во время торговли сделает ее недействительной и она не завершится.

### Добавить Предметы В Трейд

`$trade add items [идентификаторы...]`

**Проверка:** has_transaction

`<идентификаторы...>` - Идентификаторы добавляемых предметов, разделенные пробелом

Добавляет несколько предметов в сессию торговли. Предметы не могут уже быть в сессии торговли.
Предметы, которых нет в вашем инвентаре, автоматически фильтруются.
Чтобы удалить предмет, рассмотрите `$trade remove item`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Указать Сумму Трейда

`$trade set money <сумма>`

**Проверка:** has_transaction

`<сумма>` - Сумма денег, которую нужно установить, должна быть больше -1

Устанавливает сумму денег в сессии торговли. Вы не можете установить больше денег, чем у вас есть.
Чтобы добавить или удалить деньги, рассмотрите `$trade add/remove money`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Удалить Предметы Из Трейда

`$trade remove items [идентификаторы...]`

**Проверка:** has_transaction

`<идентификаторы...>` - Идентификаторы удаляемых предметов, разделенные пробелом

Удаляет несколько предметов из сессии торговли. Предметы должны уже находиться в сессии торговли.
Предметы, которых нет в сделке, автоматически фильтруются.
Чтобы удалить предмет, рассмотрите `$trade remove item`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Добавить Денег В Трейд

`$trade add money <сумма>`

**Проверка:** has_transaction

`<сумма> `- Сумма денег, которую нужно добавить, должна быть больше 0

Добавляет деньги в сессию торговли. Вы не можете добавить больше денег, чем у вас есть.
Чтобы удалить деньги, рассмотрите $trade remove money.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Снять Деньги С Трейда

`$trade remove money <сумма>`

**Проверка:** has_transaction

`<сумма>` - Сумма денег, которую нужно удалить, должна быть больше 0

Удаляет деньги из сессии торговли. Вы не можете удалить больше денег, чем вы добавили.
Чтобы добавить деньги, рассмотрите `$trade add money`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Добавить Кейсы В Трейд

`$trade add crates <количество> <редкость>`

**Проверка:** has_transaction

`<количество>` - Количество кейсов для добавления, должно быть больше 0
`<редкость>` - Редкость кейса для добавления, может быть обычной, необычной, редкой, магической или легендарной

Добавляет кейсы в сессию торговли. Вы не можете добавить больше кейсов, чем у вас есть.
Чтобы удалить кейсы, рассмотрите `$trade remove crates`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Удалить Предмет Из Трейда

`$trade remove item <идентификатор>`

**Проверка:** has_transaction

`<идентификатор>` - Идентификатор удаляемого предмета

Удаляет предмет из сессии торговли. Предмет должен быть в сделке, чтобы его удалить.
Чтобы добавить предмет, рассмотрите `$trade add item`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Добавить Предмет В Трейд

`$trade add item <идентификатор>`

**Проверка:** has_transaction

`<идентификатор>` - Идентификатор добавляемого предмета

Добавляет предмет в сессию торговли. Предмет должен быть в вашем инвентаре.
Чтобы удалить предмет, рассмотрите $trade remove item.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

###  Установить Кейсы В Трейд

`$trade set crates <количество> <редкость>`

**Проверка:** has_transaction

`<количество> `- Количество кейсов для установки, должно быть больше -1
`<редкость>` - Редкость кейса для установки, может быть обычной, необычной, редкой, магической или легендарной

Устанавливает количество кейсов в сессии торговли. Вы не можете установить больше кейсов, чем у вас есть.
Чтобы добавить или удалить кейсы, рассмотрите `$trade add/remove crates`.

*Для использования этой команды необходимо иметь открытую сессию торговли.*

### Удалить Кейсы Из Трейда

`$trade remove crates <количество> <редкость>`

**Проверка:** has_transaction

`<количество>` - Количество кейсов для удаления, должно быть больше 0
`<редкость>` - Редкость кейса для удаления, может быть обычной, необычной, редкой, магической или легендарной

Удаляет кейсы из сессии торговли. Вы не можете удалить больше кейсов, чем вы добавили.
Чтобы добавить кейсы, рассмотрите `$trade add crates`.
...
Добавляет что-то в сессию торговли.
Вы можете указать, какой предмет вы хотите добавить в торговлю с помощью метки.
Чтобы удалить предмет, рассмотрите `$trade remove`.
...
Удаляет что-то из сессии торговли.
Вы можете указать, какой предмет вы хотите удалить из торговли с помощью метки.
Чтобы добавить предмет, рассмотрите `$trade add`.
...
Удаляет что-либо из торговой сессии.
Вы можете удалить что-либо по своему выбору, используя одну из приведенных ниже подкоманд.

*Для использования этой команды у вас должна быть открытая торговая сессия.*
